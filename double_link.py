...
# to do:
#  click to move inverse kinematics done
...
class Link():
    """Object defining each link in the arm"""
...
class Arm():
    ...
    def show_arm_range(self, ax):
        range = Circle((0, 0), self.arm_length, color='red', alpha=0.5)
        ax.add_patch(range)
    ...
    def animate(self, ax, duration, frames,  new_state, static_point=np.array([0, 0]), rate=0.1):
        """Handles the smooth motion of the arm from one state to another"""
...
arm.draw(ax, np.array([[0, 0], [arm.links[0].length, 0], [
         arm.links[0].length + arm.links[1].length, 0]]))
